...
현재 프로젝트의 실제 규칙은 복사 후 프로젝트 루트에 만든 `AGENTS.md`가 우선한다.

## 문서 구성

| 문서 | 목적 |
|---|---|
| `AGENTS.template.md` | 새 프로젝트 루트에 복사해서 사용할 AI 에이전트 작업 규칙 템플릿 |
...
1. `AGENTS.template.md`를 새 프로젝트 루트의 `AGENTS.md`로 복사한다.
2. 프로젝트 이름, 검증 명령, 참고 문서 경로를 새 프로젝트에 맞게 수정한다.
3. `development-workflow.md`와 `commit-convention.md`를 새 프로젝트의 `docs/`에 복사한다.
4. Analytics를 사용하는 프로젝트라면 `analytics-guideline.md`를 복사하고, 프로젝트별 이벤트 카탈로그 문서를 별도로 만든다.
5. GitHub Issue/PR 템플릿이 필요하면 `templates/`의 파일을 `.github/` 하위 템플릿으로 옮겨 사용한다.
